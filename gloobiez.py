...
def load_done():
    # returns dict: {index: {"status": "engaged"|"claimed", "url": "..."}}
    done = {}
    ...
    with open(DONE_FILE, "r") as f:
        for line in f:
            ...
            if len(parts) >= 2:
                ...
                done[idx] = {"status": status, "url": url}
    return done

def save_done(idx, status, url=""):
    # hapus baris lama kalo ada
    lines = []
    if os.path.exists(DONE_FILE):
        with open(DONE_FILE, "r") as f:
            lines = [l for l in f.readlines() if not l.startswith(f"{idx}|")]
    lines.append(f"{idx}|{status}|{url}\n")
...
def main():
    ...
    for i in range(start, end):
        ...
        try:
            ...
            already_engaged = done.get(i, {}).get("status") in ("engaged", "claimed")
            ...
            if not already_engaged:
                ...
                print("  Skip follow/like/RT - udah engaged")
            ...
            if not comment_url:
                ...
                save_done(i, "engaged", comment_url)
            ...
            if done_flag:
                ...
                save_done(i, "claimed", comment_url)
            else:
