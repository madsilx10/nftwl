...
DONE_FILE = "done.txt"  # format: index|status|comment_url
# status: engaged = udah follow/like/rt | claimed = udah comment + submit
# ─────────────────────────────────────────────────────────────────────────────
    ...
    with open(DONE_FILE, "r") as f:
        for line in f:
            ...
            if len(parts) >= 2:
                ...
                steps = parts[3] if len(parts) >= 4 else ""
                done[idx] = {"status": status, "url": url, "steps": steps}
    return done

def save_done(idx, status, url="", steps=""):
    lines = []
    if os.path.exists(DONE_FILE):
        with open(DONE_FILE, "r") as f:
            lines = [l for l in f.readlines() if not l.startswith(f"{idx}|")]
    lines.append(f"{idx}|{status}|{url}|{steps}\n")
...
def retweet(session, tweet_id):
    ...
    if r.status_code != 200:
        raise Exception(f"RT gagal: {r.status_code} - {r.text[:300]}")
    data = r.json()
    if "errors" in data:
        raise Exception(f"RT gagal: {json.dumps(data)[:300]}")
...
def comment(session, tweet_id, text):
    ...
    if r.status_code != 200:
        raise Exception(f"Comment gagal: {r.status_code} - {r.text[:300]}")
    data = r.json()
    if "data" not in data or "create_tweet" not in data.get("data", {}):
        raise Exception(f"Comment gagal response: {json.dumps(data)[:300]}")
    tweet_results = data["data"]["create_tweet"]["tweet_results"]
    if "result" not in tweet_results:
        raise Exception(f"Comment gagal tweet_results: {json.dumps(tweet_results)[:300]}")
...
def main():
    ...
    claimed = sum(1 for v in done.values() if v["status"] == "claimed")
    print(f"\nTotal akun: {len(tokens)} | Sudah claimed: {claimed}")
    print("Mode:")
    print("  1. 1 akun")
    print("  2. Semua akun")
    ...
    for i in range(start, end):
        ...
        # Skip kalo udah claimed
        if done.get(i, {}).get("status") == "claimed":
            print(f"\n[{i+1}/{len(tokens)}] Skip - udah claimed")
            continue
        ...
        try:
            ...
            comment_url = done.get(i, {}).get("url", "")
            acc_steps = done.get(i, {}).get("steps", "")

            if "follow" not in acc_steps:
                ...
                print("  ✓ Follow")
                acc_steps += "follow,"
                save_done(i, "partial", comment_url, acc_steps)
                time.sleep(random.uniform(2, 4))
            else:
                print("  Skip follow - udah done")

            if "like" not in acc_steps:
                ...
                print("  Skip like - udah done")

            if "rt" not in acc_steps:
            ...
            if not comment_url:
                ...
                save_done(i, "engaged", comment_url, acc_steps)
                time.sleep(random.uniform(3, 6))
            else:
                print(f"  Skip komen - pakai URL: {comment_url}")
            ...
            if done_flag:
                ...
                save_done(i, "claimed", comment_url, acc_steps)
            else:
